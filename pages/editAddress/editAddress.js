...
Page({
   ...
   */
  onLoad(options) {
    const aid = options.aid; // 从页面参数获取地址ID
    if (!aid) return;
    wx.request({
      ...
      data: {
        appkey: "U2FsdGVkX19WSQ59Cg+Fj9jNZPxRC5y0xB1iV06BeNA=",
        tokenString: wx.getStorageSync('_token'),
        aid: aid // 传入地址ID
      },
      success: res => {
        console.log("res",res);
        if (res.data.code === 20000 && res.data.result[0]) {
          const address = res.data.result[0];
          this.setData({
            Name: address.name,
            Tel: address.tel,
            ...
            regionText: `${address.province}/${address.city}/${address.county}`,
